/* $Id: times_s.c,v 34.2 1995/03/29 13:04:15 ella Exp $ */
...
static char *rcsid = " $Id: times_s.c,v 34.2 1995/03/29 13:04:15 ella Exp $ ";

/* DEC/CMS REPLACEMENT HISTORY, Element TIMES_S.C */
/* *6    10-AUG-1989 12:35:18 ELLADEV "Definitive release 4.0, first provisional release 4.1. BGH" */
/* *5    30-JUN-1989 17:15:53 ELLADEV "Updated provisional with fixes for R4B383. BGH" */
/* *4    26-JUN-1989 18:08:04 ELLADEV "Updated provisional with fixes for R4B380. BGH" */
/* *3    17-FEB-1989 17:18:34 ELLADEV "Updated provisional with partial fixes for R4B043. BGH" */
/* *2    21-JAN-1989 11:24:11 ELLADEV "Provisional for Release 4.0 BGH" */
/* *1    17-JAN-1989 16:55:21 ELLADEV "first draft for Release 4.0, stored in CMS lib [elladev.cms.ella_server]""" */
/* DEC/CMS REPLACEMENT HISTORY, Element TIMES_S.C */
/* Multiplication of two arbitrary length, two's complement, signed numbers */
...
Description:
	Multiplies two two's complement integers (signed) of arbitrary length.
...
Design:
*
* Multiplication of arbitrary length, two's complement integers (signed).
*
* ASSERT X == +|x| && Y == +|y|
*
* times_s (x, y, product)
...
*      }
*
* ASSERT |product| == ABS(X) * ABS(Y)
*
*      if (sign(x) != sign(y)) negate(product);
*
* ASSERT +|product| == +|x| * +|y|
*
* }
...
void 
TIMES_S(x_known, x_addr, x_offset, x_len, 
...
#define bs_result bsalign(qf_result + qfsize)
	...
	unsigned char
				 ...
				 * workspace */
	               *first_byte_x, *first_byte_y,	/* The first bytes
							 * containing
							 * significant bits */
	...
	int             byte_length_x,	/* Number of bytes
					 ...
					 * in workspace */
	                w_bl_r, byte_length_r, sign_x, sign_y,	/* The signs of x and y */
	                padding;/* The number of padding bits in the result */
	...
	first_byte_x = row_start(x_addr, x_offset, x_len);
	first_byte_y = row_start(y_addr, y_offset, y_len);
	sign_x = getbits(*first_byte_x, (7 - (x_offset % 8)), 1);
	sign_y = getbits(*first_byte_y, (7 - (y_offset % 8)), 1);
	 ...
	 * workspace ('w_length') as 'byte_length_x * 8'.  Because
	 * 'signed_number' is TRUE, all left padding bits are given the value
	 * of the sign bit.
	 */

	adjust_input(x_addr, x_offset, x_len, w_x,
		     (w_bl_x * 8), signed_number);
	...
	if (w_bl_r > byte_length_r)
		w_r++;
#endif
	adjust_output(adjustaddress(bs_result, byte_length_r),
	   w_r, padding, byte_length_r);
}				/* times_s */
