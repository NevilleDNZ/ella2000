/* $Id: times_real.c,v 34.2 1995/03/29 13:04:14 ella Exp $ */
...
static char *rcsid = " $Id: times_real.c,v 34.2 1995/03/29 13:04:14 ella Exp $ ";

/* DEC/CMS REPLACEMENT HISTORY, Element TIMES_REAL.C */
/* *7    11-AUG-1989 15:05:27 ELLADEV "Updated provisional with fixes for R4B388. BGH" */
/* *6    10-AUG-1989 12:35:14 ELLADEV "Definitive release 4.0, first provisional release 4.1. BGH" */
/* *5    30-JUN-1989 17:15:50 ELLADEV "Updated provisional with fixes for R4B383. BGH" */
/* *4    26-JUN-1989 18:08:00 ELLADEV "Updated provisional with fixes for R4B380. BGH" */
/* *3    17-FEB-1989 17:18:31 ELLADEV "Updated provisional with partial fixes for R4B043. BGH" */
/* *2    21-JAN-1989 11:24:09 ELLADEV "Provisional for Release 4.0 BGH" */
/* *1    17-JAN-1989 16:55:16 ELLADEV "first draft for Release 4.0, stored in CMS lib [elladev.cms.ella_server]""" */
/* DEC/CMS REPLACEMENT HISTORY, Element TIMES_REAL.C */
/************************************************************************/
...
if (n_known && m_known)
   {
     /* Set up default results to return should an exception occur */
  ...
  ULTRIX_FPE_GUARD_END
   ...
   /* If we get this far on VAX, no exceptions occurred, so signal this */
   ...
   if (errEvt != 0)
     ...
     *(qf)qf_result  = TRUE;
     ...
     if (*(tv)tv_result  == twovalued_QUERY)
       {
       /* For '*' a zero answer is valid if either input was zero */

       if ( (*m_addr==0)||(*n_addr==0) )
         {
         *(tv)tv_result  = twovalued_TRUE;
         }
       else
